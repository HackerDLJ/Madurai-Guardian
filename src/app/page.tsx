            ...
            <div className="flex flex-wrap justify-start gap-4 w-full pt-4">
              {socialLinks.map((link) => (
                <Button 
                  ...
                  variant="outline"
                  className={cn(
                    "rounded-[24px] w-24 h-24 bg-white/5 border-white/10 backdrop-blur-xl shadow-lg transition-all duration-500 group/btn flex flex-col items-center justify-center gap-2",
                    link.color
                  )}
                >
                  <a href={link.href} target="_blank" rel="noopener noreferrer">
                    {link.icon}
                    <span className="text-[9px] font-black uppercase tracking-[0.15em] opacity-0 group-hover/btn:opacity-100 transition-opacity">{link.label}</span>
                  </a>
                </Button>
              ))}
            </div>
